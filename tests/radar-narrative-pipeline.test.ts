...
function grokJson(patch: Record<string, unknown> = {}) {
  return JSON.stringify({ status: "COMPLETED", decision: "HOLD", confidence: 0.72, narrative_score: 61, risk_score: 39, freshness_score: 58, sentiment_score: 64, lead_score: 44, stage: "扩散", summary: "Harvest农场题材正在小范围扩散，存在社区传播但缺少强新闻催化。", positive_reasons: ["社区传播"], negative_reasons: ["催化有限"], invalidators: [], recommended_action: "HOLD", evidence: [{ url: "https://example.com/post", title: "signal-time post", published_at: "2026-09-22T09:40:00.000Z", relation: "support", reason: "早于信号" }], ...patch });
}
...
describe("radar narrative pipeline", () => {
  ...
  it("drops post-signal evidence and refuses to manufacture a score", () => {
    const review = parseRadarAiReview(grokJson({ evidence: [{ url: "https://example.com/future", title: "future", published_at: "2026-09-22T09:50:00.000Z", relation: "support", reason: "too late" }] }), { inputCutoffAt: cutoff });
    expect(review.status).toBe("INSUFFICIENT_EVIDENCE"); expect(review.narrative_score).toBeNull(); expect(review.evidence).toEqual([]);
  });
  ...
  it("recognizes rate limits and retries once before success", async () => {
    ...
    expect(fetcher).toHaveBeenCalledTimes(2); expect(review.status).toBe("COMPLETED"); expect(review.attempt_count).toBe(2);
  });
  ...
  it("uses an idempotent recent-50 backfill query and preserves discovery time", () => {
    ...
    expect(source).toContain("NOT EXISTS"); expect(source).toContain("n.status='COMPLETED'"); expect(source).toContain("Math.min(50");
