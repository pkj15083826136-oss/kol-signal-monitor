...
describe("trading feature flags", () => {
  it("fails closed when configuration is absent or malformed", () => {
    expect(tradingFeatureFlags({})).toEqual({ walletConnect: false, liveMarket: false, tradeQuote: false, tradeTestnet: false, tradeMainnet: false, radarWalletLogin: false, radarAutoTrade: false, tradeMainnetChains: { sol: false, bsc: false, base: false, robinhood: false } });
  ...
  it("enables only explicit true values", () => {
    ...
    expect(flags.radarWalletLogin).toBe(false);
    expect(flags.radarAutoTrade).toBe(false);
  });
});
