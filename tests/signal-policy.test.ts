...
describe("first signal policy", () => {
  ...
  it("applies the configurable high-market-cap ceiling before age", () => {
    expect(rejectFirstSignal({ symbol: "NEW", marketCap: FIRST_SIGNAL_MAX_MARKET_CAP - 1, createdAt: now - FIRST_SIGNAL_MAX_AGE_MS - 1, now })).toBe(false);
    expect(rejectFirstSignal({ symbol: "NEW", marketCap: FIRST_SIGNAL_MAX_MARKET_CAP, createdAt: now, now })).toBe(true);
    expect(rejectFirstSignal({ symbol: "NEW", marketCap: FIRST_SIGNAL_MAX_MARKET_CAP + 1, createdAt: now, now })).toBe(true);
  });
