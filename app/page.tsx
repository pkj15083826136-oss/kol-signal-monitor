...
const demoSignals: SignalRow[] = [
  { id: 0, chain: "sol", tokenAddress: "Dz9mQ9NzkBcCsuGPFJ3r1bS4wgqKMHBPiVuniW8Mbonk", name: "USELESS", symbol: "USELESS", threshold: 38, holderCount: 42, marketCap: 251100000, liquidity: 4600000, holders: 68100, volume24h: 18500000, aiAnalysis: "鲸鱼资金持续流入，叙事围绕“无用即价值”的反讽文化扩散；热度较强，但需防范高位集中兑现。", gmgnTheme: "Solana 社区文化 Meme", alertedAt: new Date().toISOString(), walletNames: ["cented", "Cupsey", "Tahi", "聪明钱包"], demo: true },
  { id: -1, chain: "base", tokenAddress: "0x71f…d30c", name: "Based Signal", symbol: "SIGNAL", threshold: 18, holderCount: 21, marketCap: 640000, liquidity: 72000, holders: 2840, volume24h: 910000, aiAnalysis: "社区正在传播AI代理与链上信号叙事，买盘扩散速度较快；有效原创内容仍偏少。", gmgnTheme: "AI Agent Meme", alertedAt: new Date(Date.now() - 18 * 60_000).toISOString(), walletNames: ["KOL-A", "KOL-B", "聪明钱包"], demo: true },
];

async function loadSignals(): Promise<{ signals: SignalRow[]; lastRun: string | null; monitorOk: boolean }> {
  try {
    const rows = await env.DB.prepare(`SELECT id, chain, token_address, name, symbol, threshold, holder_count, market_cap,
    ...
    return {
      signals: rows.results.map((row) => ({
        id: Number(row.id), chain: String(row.chain), tokenAddress: String(row.token_address), name: String(row.name), symbol: String(row.symbol),
