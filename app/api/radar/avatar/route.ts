...
export async function GET(request: Request) {
  ...
  if (source) try {
    const upstream = await fetch(source, { redirect: "manual", headers: { Accept: "image/png,image/jpeg,image/webp,image/gif" }, signal: AbortSignal.timeout(10_000) }); const type = upstream.headers.get("content-type"); const declared = Number(upstream.headers.get("content-length") || 0);
  ...
  } catch (error) { console.warn("RADAR_AVATAR_PROXY_FAILED", { host: source ? new URL(source).hostname : "invalid", code: error instanceof DOMException ? error.name : error instanceof Error ? error.message.slice(0, 80) : "unknown" }); }
