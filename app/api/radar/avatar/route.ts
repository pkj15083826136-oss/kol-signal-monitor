...
export async function GET(request: Request) {
  ...
  if (source) try {
    const upstream = await fetch(source, { redirect: "error", headers: { Accept: "image/png,image/jpeg,image/webp,image/gif" }, signal: AbortSignal.timeout(5_000) }); const type = upstream.headers.get("content-type"); const declared = Number(upstream.headers.get("content-length") || 0);
    if (upstream.ok && validAvatarContentType(type) && (!declared || declared <= MAX_BYTES) && upstream.body) { const reader = upstream.body.getReader(); const chunks: Uint8Array[] = []; let size = 0; while (true) { const part = await reader.read(); if (part.done) break; size += part.value.byteLength; if (size > MAX_BYTES) { await reader.cancel(); throw new Error("AVATAR_TOO_LARGE"); } chunks.push(part.value); } const body = new Uint8Array(size); let offset = 0; for (const chunk of chunks) { body.set(chunk, offset); offset += chunk.byteLength; } return new Response(body, { headers: { "Content-Type": String(type).split(";")[0], "Cache-Control": "public, max-age=3600, s-maxage=86400", "X-Content-Type-Options": "nosniff" } }); }
