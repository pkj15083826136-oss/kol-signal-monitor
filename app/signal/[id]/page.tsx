...
export default async function SignalDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [posts, snapshots, market, candles] = await Promise.all([
    ...
    getMarketData(chain, address).catch(() => ({ name:"",symbol:"",logo:"",description:"",price:0,marketCap:0,liquidity:0,holders:0,volume24h:0,pairAddress:"",dexUrl:"" })),
    getKlineData(chain, address).catch(() => []),
  ]);
  const livePrice = market.price || Number(signal.price);
  const chartData = snapshots.results.map((row) => ({
    time: new Date(String(row.captured_at)).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" }),
  ...
  const description = market.description || String(signal.gmgn_theme || "暂无公开项目介绍");
  return <main className="min-h-screen bg-[#070a0f] text-[#edf2f7]"><div className="ambient"/><div className="relative mx-auto max-w-6xl px-4 py-7 sm:px-7"><Link href="/" className="mb-6 inline-flex items-center gap-2 text-sm text-slate-500 transition hover:text-cyan-300"><ArrowLeft size={16}/>返回预警列表</Link>
    <section className="rounded-2xl border border-white/[0.08] bg-[#0b1018]/95 p-5 sm:p-7"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start"><div className="flex min-w-0 items-center gap-3">{validImage(logo) ? <img src={logo} alt={`${symbol} 头像`} className="h-12 w-12 shrink-0 rounded-xl border border-white/[0.08] bg-white/[0.04] object-cover"/> : <div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl border border-cyan-300/15 bg-cyan-300/[0.08] font-semibold text-cyan-200">{symbol.slice(0,2)}</div>}<div className="min-w-0"><h1 className="truncate text-2xl font-semibold">{name} <span className="text-slate-500">${symbol}</span></h1><div className="mt-1 flex items-center gap-2"><p className="min-w-0 truncate font-mono text-xs text-slate-600">{address}</p><CopyAddress address={address}/></div></div></div><div className="rounded-xl border border-cyan-300/15 bg-cyan-300/[0.07] px-4 py-3"><div className="text-xs text-cyan-300/70">当前聚集</div><div className="mt-1 text-xl font-semibold text-cyan-200">{Number(signal.holder_count)} 人</div></div></div>
      ...
      <div className="mt-7 grid gap-3 lg:grid-cols-[1.1fr_1fr]"><div className="rounded-xl border border-white/[0.07] bg-white/[0.025] p-5"><div className="text-sm font-medium text-slate-300">代币简介</div><p className="mt-2 text-sm leading-6 text-slate-400">{description}</p>{market.dexUrl && <a href={market.dexUrl} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1 text-xs text-cyan-300/80 hover:text-cyan-200">查看DEX行情 <ExternalLink size={12}/></a>}</div><div className="rounded-xl border border-cyan-300/10 bg-gradient-to-r from-cyan-300/[0.07] to-violet-400/[0.05] p-5"><div className="flex items-center gap-2 text-sm text-cyan-300"><Sparkles size={16}/>AI分析</div><p className="mt-2 leading-7 text-slate-200">{String(signal.ai_analysis)}</p><p className="mt-3 text-xs text-slate-500">数据主题：{String(signal.gmgn_theme)}</p></div></div>
    </section>
    <section className="mt-5 rounded-2xl border border-white/[0.08] bg-[#0b1018]/95 p-5"><div className="flex items-center justify-between"><div><h2 className="font-semibold">价格K线</h2><p className="mt-1 text-xs text-slate-600">主流动池 · 5分钟 · 最近约10小时</p></div><span className="text-xs text-slate-600">按打开详情页加载</span></div><KlineChart data={candles}/></section>
    <section className="mt-5 grid gap-5 lg:grid-cols-[1.65fr_1fr]"><div className="rounded-2xl border border-white/[0.08] bg-[#0b1018]/95 p-5"><div className="mb-2 flex items-center justify-between"><div><h2 className="font-semibold">KOL持仓趋势</h2><p className="mt-1 text-xs text-slate-600">持仓人数与按当前行情估算总价值</p></div><span className="text-xs text-slate-600">自动更新</span></div>{chartData.length ? <SignalChart data={chartData}/> : <div className="grid h-72 place-items-center text-sm text-slate-600">等待下一轮持仓快照</div>}</div><div className="rounded-2xl border border-white/[0.08] bg-[#0b1018]/95 p-5"><div className="flex items-center gap-2"><Users size={16} className="text-cyan-300"/><h2 className="font-semibold">建仓账户</h2></div><div className="mt-4 flex max-h-72 flex-wrap content-start gap-2 overflow-auto">{wallets.map((name, index) => <span key={`${name}-${index}`} className="rounded-lg border border-white/[0.08] bg-white/[0.03] px-2.5 py-1.5 text-xs text-slate-400">{name}</span>)}</div></div></section>
