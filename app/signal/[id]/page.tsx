...
export default async function SignalDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [posts, snapshots, market, kline15] = await Promise.all([
    ...
    db.prepare("SELECT holder_count, total_token_amount, market_value, captured_at FROM snapshots WHERE chain = ? AND token_address = ? AND datetime(captured_at) >= datetime('now', '-24 hours') ORDER BY captured_at ASC LIMIT 600").bind(chain, address).all<Record<string, unknown>>(),
