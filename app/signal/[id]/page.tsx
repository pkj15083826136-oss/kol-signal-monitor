...
function genericIntro(value: string) { return !value || value.includes("等待社媒叙事确认") || value.includes("暂无明确项目介绍"); }
...
export default async function SignalDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [posts, snapshots, market, kline] = await Promise.all([
    ...
    getKlineData(chain, address).catch(() => ({ candles: [], source: "", reason: "行情数据源请求失败，请稍后刷新重试。" })),
  ]);
  ...
  const storedIntro = String(signal.gmgn_theme || "");
  const description = market.description || (genericIntro(storedIntro) ? `AI补充简介：${String(signal.ai_analysis)}` : storedIntro);
  return <main className="min-h-screen bg-[#070a0f] text-[#edf2f7]"><div className="ambient"/><div className="relative mx-auto max-w-6xl px-4 py-7 sm:px-7"><a href="/" className="mb-6 inline-flex items-center gap-2 text-sm text-slate-500 transition hover:text-cyan-300"><ArrowLeft size={16}/>返回预警列表</a>
    ...
    <section className="mt-5 rounded-2xl border border-white/[0.08] bg-[#0b1018]/95 p-5"><div className="flex items-center justify-between"><div><h2 className="font-semibold">价格K线</h2><p className="mt-1 text-xs text-slate-600">5分钟 · 最近约10小时</p></div><span className="text-xs text-slate-600">{kline.source || "暂无数据源"}</span></div>{kline.candles.length ? <KlineChart data={kline.candles}/> : <div className="mt-4 grid h-64 place-items-center rounded-xl border border-dashed border-white/[0.08] px-6 text-center text-sm leading-6 text-slate-500">{kline.reason}</div>}</section>
