...
import FlowDashboard, { type FlowAggregate, type FlowEvent, type FlowHealth, type FlowPayload, type MintEvent } from "./flow-dashboard";

export const dynamic = "force-dynamic";

async function load(): Promise<FlowPayload> {
  if (!env.DB) return { events: [], mintEvents: [], aggregates: [], health: [], recordCount: 0, totalVerifiedRecords: 0, latestEventAt: null, latestDiscoveredAt: null, window: "24h", minimumUsd: 10_000_000, signConvention: "净额=流出-流入", institutionTradeCoverage: "覆盖不足", fetchedAt: new Date().toISOString() };
  ...
  const [events, mintEvents, aggregates, health, filteredStats, totalStats] = await Promise.all([
    ...
    env.DB.prepare("SELECT * FROM stablecoin_mint_events WHERE chain_occurred_at>=? AND verification_status='verified_supply_increase' ORDER BY chain_occurred_at DESC LIMIT 100").bind(mintFrom).all<MintEvent>(),
