...
import { BellRing, ChevronRight, Clock3, Database, Flame, History, Landmark, Radar, RadioTower, Search, Sparkles, TrendingUp, Users } from "lucide-react";
...
export default function Dashboard({ signals: initialSignals, initialNextCursor, walletCount, lastRun: initialLastRun, monitorOk: initialMonitorOk, demo, liveMarketEnabled, chainHealth: initialChainHealth, sourceHealth: initialSourceHealth, alertSummary: initialAlertSummary, initialContinuity }: { signals: SignalRow[]; initialNextCursor: string | null; walletCount: number; lastRun: string | null; monitorOk: boolean; demo: boolean; liveMarketEnabled: boolean; chainHealth: ChainHealth[]; sourceHealth: SourceHealth[]; alertSummary: AlertSummary; initialContinuity: ContinuityStatus }) {
  ...
  const [visibleIds, setVisibleIds] = useState<Set<number>>(() => new Set(initialSignals.slice(0, 20).map((signal) => signal.id)));
  const sentinel = useRef<HTMLDivElement>(null);
  const updateVisibility = useCallback((id: number, visible: boolean) => setVisibleIds((current) => { const has = current.has(id); if (has === visible) return current; const next = new Set(current); if (visible) next.add(id); else next.delete(id); return next; }), []);
  ...
  useEffect(() => {
    ...
    async function refresh() {
      try {
        const params = new URLSearchParams(); if (chain !== "all") params.set("chain", chain); if (query.trim()) params.set("q", query.trim());
        const response = await fetch(`/api/signals?${params}`, { cache: "no-store" });
        if (!response.ok) return;
        const payload = await response.json() as { signals: SignalRow[]; nextCursor: string | null; lastRun: string | null; monitorOk: boolean; chainHealth: ChainHealth[]; sourceHealth: SourceHealth[]; alertSummary: AlertSummary };
        if (!active) return;
        ...
        setSignals((current) => { const older = current.filter((signal) => !payload.signals.some((freshSignal) => freshSignal.id === signal.id)); return [...payload.signals, ...older]; }); setNextCursor(payload.nextCursor); setLastRun(payload.lastRun); setMonitorOk(payload.monitorOk); setChainHealth(payload.chainHealth); setSourceHealth(payload.sourceHealth); setAlertSummary(payload.alertSummary);
  ...
  async function loadMore() {
    if (!nextCursor || loadingMore) return; setLoadingMore(true); setLoadError(false);
    try { const params = new URLSearchParams({ cursor: nextCursor }); if (chain !== "all") params.set("chain", chain); if (query.trim()) params.set("q", query.trim()); const response = await fetch(`/api/signals?${params}`, { cache: "no-store" }); if (!response.ok) throw new Error(); const payload = await response.json() as { signals: SignalRow[]; nextCursor: string | null }; setSignals((current) => [...current, ...payload.signals.filter((row) => !current.some((existing) => existing.id === row.id))]); setNextCursor(payload.nextCursor); } catch { setLoadError(true); } finally { setLoadingMore(false); }
  }
  useEffect(() => { const node = sentinel.current; if (!node) return; const observer = new IntersectionObserver((entries) => { if (entries[0]?.isIntersecting) void loadMore(); }, { rootMargin: "300px" }); observer.observe(node); return () => observer.disconnect(); });
  ...
  const liveTokens = useMemo(() => signals.filter((signal) => visibleIds.has(signal.id)).slice(0, 30).map((signal) => ({ chain: signal.chain, address: signal.tokenAddress })), [signals, visibleIds]);
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#070a0f] text-[#edf2f7]">
    ...
    <header className="sticky top-0 z-30 border-b border-white/[0.07] bg-[#070a0f]/85 backdrop-blur-xl"><div className="mx-auto flex min-h-16 max-w-[1500px] flex-wrap items-center justify-between gap-2 px-4 py-2 sm:px-7"><div className="flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-xl border border-cyan-300/20 bg-cyan-300/10 text-cyan-300"><RadioTower size={18} /></div><div><div className="font-semibold tracking-tight">KOL Signal</div><div className="text-[11px] tracking-[0.18em] text-slate-500">SMART FLOW MONITOR</div></div></div><div className="flex flex-wrap items-center justify-end gap-2 text-sm"><a href="/history" className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-2.5 py-1.5 text-xs text-slate-400 hover:text-slate-200"><History size={14}/><span className="hidden md:inline">历史</span></a><a href="/radar" className="inline-flex items-center gap-1.5 rounded-lg border border-cyan-300/15 bg-cyan-300/[0.06] px-2.5 py-1.5 text-xs text-cyan-200 hover:bg-cyan-300/[0.1]"><Radar size={14}/><span className="hidden sm:inline">土狗雷达</span></a><a href="/exchanges" className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-2.5 py-1.5 text-xs text-slate-300 hover:border-cyan-300/20 hover:text-cyan-200"><Landmark size={14}/><span>交易所动态</span></a><a href="/flows" className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-2.5 py-1.5 text-xs text-slate-300 hover:border-violet-300/20 hover:text-violet-200"><TrendingUp size={14}/><span>大额资金流向</span></a><span className={`hidden items-center gap-2 rounded-full border px-3 py-1.5 xl:flex ${continuityOk ? "border-emerald-300/15 bg-emerald-300/[0.07] text-emerald-300" : "border-amber-300/15 bg-amber-300/[0.07] text-amber-300"}`}><span className={`h-1.5 w-1.5 rounded-full ${continuityOk ? "bg-emerald-300 pulse" : "bg-amber-300"}`} />{continuityOk ? "运行正常" : continuity.github.sync === "diverged" ? "源码分叉" : continuity.github.schedule === "stopped" ? "调度停止" : "调度延迟"}</span><WalletButton/></div></div></header>
    <div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-7 sm:py-8">
      ...
      <section className="mt-6 overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0b1018]/90 shadow-2xl shadow-black/20">
        ...
        <div className="divide-y divide-white/[0.06]">{filtered.map((signal) => <SignalCard key={signal.id} signal={signal} fresh={newIds.includes(signal.id)} live={live.items[marketKey(signal.chain, signal.tokenAddress)]} now={clockNow} onVisibility={updateVisibility} />)}{!filtered.length && <div className="grid min-h-60 place-items-center p-8 text-center"><div><Database className="mx-auto mb-3 text-slate-700" /><p className="text-slate-300">没有匹配的预警</p></div></div>}</div>
        <div ref={sentinel} className="border-t border-white/[0.06] px-5 py-4 text-center text-xs text-slate-500">{loadingMore ? "正在加载更多…" : loadError ? <button type="button" onClick={() => void loadMore()} className="text-amber-300 hover:text-amber-200">加载失败，点击重试</button> : nextCursor ? "继续下滑加载更多" : "已到底"}</div>
...
function SystemStatus({ chains, sources, alerts }: { chains: ChainHealth[]; sources: SourceHealth[]; alerts: AlertSummary }) {
  ...
  const sourceErrors = enabled.filter((source) => ["error", "unavailable", "unknown"].includes(source.state)).length;
  const deliveryAttention = alerts.pending + alerts.retry;
...
function SignalCard({ signal, fresh, live, now, onVisibility }: { signal: SignalRow; fresh: boolean; live?: LiveMarketItem; now: number; onVisibility: (id: number, visible: boolean) => void }) {
  const cardRef = useRef<HTMLElement>(null);
  useEffect(() => { const node = cardRef.current; if (!node) return; const observer = new IntersectionObserver((entries) => onVisibility(signal.id, Boolean(entries[0]?.isIntersecting)), { rootMargin: "100px" }); observer.observe(node); return () => { observer.disconnect(); onVisibility(signal.id, false); }; }, [onVisibility, signal.id]);
  ...
  return <article ref={cardRef} className={`group grid min-w-0 gap-4 overflow-hidden p-4 transition sm:px-5 xl:min-h-28 xl:grid-cols-[minmax(220px,0.9fr)_minmax(280px,1.05fr)_minmax(640px,1.8fr)] xl:items-center ${fresh ? "bg-cyan-300/[0.07] ring-1 ring-inset ring-cyan-300/15" : "hover:bg-white/[0.025]"}`}>
