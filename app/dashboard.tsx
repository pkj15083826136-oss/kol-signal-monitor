...
import { BellRing, ChevronRight, Clock3, Database, Flame, History, Radar, RadioTower, Search, Sparkles, Users } from "lucide-react";
...
function SystemStatus({ chains, sources, alerts }: { chains: ChainHealth[]; sources: SourceHealth[]; alerts: AlertSummary }) {
  ...
  const enabled = sources.filter((source) => !["off", "unconfigured"].includes(source.state)); const sourceHealthy = enabled.filter((source) => source.state === "healthy").length;
  ...
  return <section className="mb-4 flex min-w-0 flex-col gap-3 rounded-2xl border border-white/[0.08] bg-[#0b1018]/90 px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
    ...
    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500"><details className="relative"><summary className="cursor-pointer list-none">数据源 {sourceHealthy}/{enabled.length} 正常</summary><div className="absolute right-0 z-20 mt-2 max-h-72 w-[min(82vw,420px)] overflow-auto rounded-xl border border-white/10 bg-[#0b1018] p-3 shadow-2xl">{enabled.map((source) => <div key={`${source.source}:${source.chain}`} className="flex min-w-0 justify-between gap-3 border-b border-white/5 py-2"><span className="min-w-0 truncate">{source.source} · {source.chain}</span><span className="shrink-0">{source.state} · {source.lastSuccessAt ? formatSignalAge(source.lastSuccessAt) : "未成功"}</span></div>)}</div></details><details className="relative"><summary className={`cursor-pointer list-none ${attention ? "text-amber-300" : "text-slate-500"}`}>通知待处理 {attention}</summary><div className="absolute right-0 z-20 mt-2 w-64 rounded-xl border border-white/10 bg-[#0b1018] p-3 shadow-2xl">pending {alerts.pending} · retry {alerts.retry} · manual_review {alerts.manualReview}<p className="mt-2 leading-5 text-slate-600">雷达 threshold=0 使用 radar_visible 状态，不进入企业微信发送队列。</p></div></details></div>
  </section>;
}
