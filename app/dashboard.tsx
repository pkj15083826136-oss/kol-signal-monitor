...
function TokenAvatar({ signal, compact=false }: { signal: SignalRow; compact?: boolean }) { const cls = compact ? "h-9 w-9 rounded-lg" : "h-11 w-11 rounded-xl"; return validImage(signal.logo) ? <img src={signal.logo} alt="" className={`${cls} shrink-0 border border-white/[0.08] bg-white/[0.04] object-cover`}/> : <div className={`grid ${cls} shrink-0 place-items-center border border-white/[0.08] bg-white/[0.04] font-semibold text-cyan-200`}>{signal.symbol.slice(0,2)}</div>; }
...
function SignalCard({ signal, fresh }: { signal: SignalRow; fresh: boolean }) { return <a href={`/signal/${signal.id}`} className={`group grid cursor-pointer gap-4 p-4 transition sm:px-5 lg:grid-cols-[minmax(220px,1.05fr)_minmax(300px,1.7fr)_auto] lg:items-center ${fresh ? "bg-cyan-300/[0.07] ring-1 ring-inset ring-cyan-300/15" : "hover:bg-white/[0.025]"}`}><div className="flex items-center gap-3"><TokenAvatar signal={signal}/><div className="min-w-0"><div className="flex items-center gap-2"><strong className="truncate font-semibold">{signal.symbol}</strong><span className={`rounded-md border px-1.5 py-0.5 text-[11px] ${chainTone[signal.chain] || chainTone.base}`}>{chainName[signal.chain] || signal.chain}</span>{fresh && <span className="text-[11px] text-cyan-300">新信号</span>}</div>{signal.name && signal.name.toLowerCase() !== signal.symbol.toLowerCase() && <div className="mt-0.5 truncate text-xs text-slate-500">{signal.name}</div>}<div className="mt-1 font-mono text-xs text-slate-600">{shortAddress(signal.tokenAddress)}</div></div></div><div><div className="flex items-center gap-2 text-xs text-slate-500"><Sparkles size={13} className="text-cyan-400" /><span>AI分析</span><span className="text-slate-700">·</span><span className="truncate">{signal.gmgnTheme}</span></div><p className="mt-1.5 line-clamp-2 text-sm leading-6 text-slate-300">{signal.aiAnalysis}</p></div><div className="flex items-center justify-between gap-4 lg:justify-end"><div className="grid grid-cols-4 gap-4 text-right"><Stat label="KOL" value={`${signal.holderCount}人`} bright /><Stat label="持币地址" value={signal.holders ? signal.holders.toLocaleString() : "待补全"} /><Stat label="市值" value={money(signal.marketCap)} /><Stat label="流动性" value={money(signal.liquidity)} /></div><div className="hidden items-center gap-2 text-xs text-slate-600 2xl:flex"><Clock3 size={13} />{timeAgo(signal.alertedAt)}</div><ChevronRight size={18} className="text-slate-700 transition group-hover:translate-x-0.5 group-hover:text-cyan-300" /></div></a>; }
function Stat({ label, value, bright }: { label: string; value: string; bright?: boolean }) { return <div><div className="text-[11px] text-slate-600">{label}</div><div className={`mt-1 whitespace-nowrap text-sm font-medium ${bright ? "text-cyan-300" : "text-slate-300"}`}>{value}</div></div>; }
