...
export type SignalRow = {
  ...
  createdAt: string; walletNames: string[]; demo?: boolean;
};
...
function SignalCard({ signal, fresh, live, now }: { signal: SignalRow; fresh: boolean; live?: LiveMarketItem; now: number }) {
  ...
  const liveMoney = (value: number | undefined) => available ? formatUsdCompact(value) : "--";
  const holders = available && live?.holders ? live.holders : signal.holders;
  ...
  return <article className={`group grid min-w-0 gap-4 overflow-hidden p-4 transition sm:px-5 xl:min-h-28 xl:grid-cols-[minmax(220px,0.9fr)_minmax(280px,1.05fr)_minmax(640px,1.8fr)] xl:items-center ${fresh ? "bg-cyan-300/[0.07] ring-1 ring-inset ring-cyan-300/15" : "hover:bg-white/[0.025]"}`}>
    <div className="flex min-w-0 items-center gap-3"><a href={href} onClick={remember} aria-label={`查看 ${signal.symbol} 详情`} className="shrink-0"><TokenAvatar signal={signal}/></a><div className="min-w-0 flex-1"><div className="flex min-w-0 items-center gap-2"><a href={href} onClick={remember} className="truncate font-semibold hover:text-cyan-200">{signal.symbol}</a><span className={`shrink-0 rounded-md border px-1.5 py-0.5 text-[11px] ${chainTone(signal.chain)}`}>{chainLabel(signal.chain)}</span>{fresh && <span className="shrink-0 text-[11px] text-cyan-300">新信号</span>}</div><div className="mt-1 flex min-w-0 items-center gap-1.5"><span className="min-w-0 flex-1 truncate font-mono text-xs text-slate-500">{shortAddress(signal.tokenAddress)}</span><CopyAddress address={signal.tokenAddress} compact/>{stale && <span className="shrink-0 text-[10px] text-amber-300">行情延迟</span>}</div></div></div>
