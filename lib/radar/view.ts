...
import { publicEnrichmentStatus } from "@/lib/radar/public-status";
...
function gmgnCategory(endpoint: unknown) { const value = String(endpoint || ""); if (value.includes("smartmoney")) return "Smart Money"; if (value.includes("/kol")) return "KOL"; if (value.includes("token/info")) return "Token基础信息"; if (/pool|market/i.test(value)) return "流动池/市场信息"; return "其他数据"; }
...
function mapRadar(row: Record<string, unknown>): RadarViewRow {
  return {
    ...
    avatarUrl: String(row.avatar_resolved_url || identiconUrl(String(row.chain), String(row.token_address))), avatarStatus: String(row.avatar_status || "PENDING"), enrichmentStatus: String(row.enrichment_status || "PENDING"), enrichmentError: row.enrichment_error_code || row.enrichment_status ? publicEnrichmentStatus(String(row.enrichment_error_code || row.enrichment_status)) : null, tradeDataStage: String(row.trade_data_stage || "CANDIDATE_ONLY"), pairStatus: String(row.pair_status || "PENDING"), pairSource: row.pair_source ? String(row.pair_source) : null, liquidityStatus: String(row.liquidity_status || "PENDING"), liquiditySource: row.liquidity_source ? String(row.liquidity_source) : null,
...
export async function loadRadarDashboard(db: D1Database, options: { offset?: number; limit?: number } = {}): Promise<RadarDashboardData> {
  ...
  const [radar, totals, positions, orders, collector, gmgnHealth, sampleGroups, manualCases, promptVersions] = await Promise.all([
    db.prepare(`SELECT r.*, (SELECT source FROM radar_signal_sources s WHERE s.radar_signal_id=r.id ORDER BY observed_at ASC LIMIT 1) signal_source,
      ...
      e.status enrichment_status,e.trade_data_stage,e.avatar_resolved_url,e.avatar_status,e.error_code enrichment_error_code,e.primary_pair_address,e.pair_status,e.pair_source,e.liquidity_usd enrichment_liquidity,e.liquidity_status,e.liquidity_source,e.unique_buyers_24h,e.unique_sellers_24h,e.buy_tx_24h,e.sell_tx_24h,e.token_created_at,e.system_first_seen_at,
