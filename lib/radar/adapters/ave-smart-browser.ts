...
export class AveSmartEventBuffer {
  ...
  ingest(rows: unknown[], observedAt = new Date().toISOString()) {
    ...
    for (const value of rows) {
      ...
      candidates.push({ source: "ave_smart_browser", sourceEventId: id, chain, tokenAddress: token, pairAddress: typeof row.amm === "string" ? row.amm : null, name: String(row.headline || row.symbol || "Unknown"), symbol: String(row.symbol || "—"), firstSeenAt: signalTime, poolCreatedAt: isoOrNull(row.token_create_time), price: numericString(row.current_price_usd), marketCap: numeric(row.mc_cur ?? row.mc), liquidity: null, volume24h: numeric(row.tx_volume_u_24h), holders: numeric(row.holders_cur ?? row.holders), buyers: null, sellers: null, smartMoneyCount: Math.max(0, Math.round(numeric(row.action_count) || 0)), dataFetchedAt: observedAt, identityVerified: false, sellSimulationPassed: null, honeypot: null, mintable: null, freezable: null, blacklistable: null, taxModifiable: null, buyTaxBps: null, sellTaxBps: null, lpLocked: null, topHolderPct: numeric(row.top10_ratio), developerRisk: "unknown", priceImpactBps: null, sourceConflict: false, rawSnapshot: { id, token, chain: row.chain, signal_time: row.signal_time, first_signal_time: row.first_signal_time, signal_type: row.signal_type, tag: row.tag, token_tag: row.token_tag, issue_platform: row.issue_platform, first_signal_mc: row.first_signal_mc, mc_cur: row.mc_cur, holders_cur: row.holders_cur, top10_ratio: row.top10_ratio, current_price_usd: row.current_price_usd } });
    }
    return candidates;
  }
}
