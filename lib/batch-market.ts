...
import { OkxMarketClient, OkxRequestError, type OkxCredentials, type OkxRequestMeta, type OkxResult } from "@/lib/providers/okx";
...
function clientFor(credentials: OkxCredentials | null | undefined) {
...
export async function getBatchMarketData(tokens: Array<{ chain: string; address: string }>, credentials?: OkxCredentials | null): Promise<BatchMarketItem[]> {
  ...
  const [batches, okxResult] = await Promise.all([Promise.all([...groups].map(async ([chain, addresses]): Promise<DexPair[]> => {
    try {
  ...
  })), clientFor(credentials)?.priceInfo(tokens.slice(0, 30)).catch((error): OkxResult<Map<string, TokenMarketCandidate>> => ({ value: new Map(), meta: { requestCount: 1, latencyMs: 0, cacheHit: false, rateLimited: error instanceof OkxRequestError && error.kind === "rate_limited", status: error instanceof OkxRequestError && error.kind === "rate_limited" ? "rate_limited" : "unavailable" } })) ?? Promise.resolve(null)]);
  const okx = okxResult?.value ?? new Map<string, TokenMarketCandidate>();
  ...
  return tokens.slice(0, 30).map((token) => {
    ...
    const okxItem = okx.get(`${token.chain}:${token.chain === "sol" ? token.address : token.address.toLowerCase()}`);
    const resolved = resolveTokenMarket([okxItem, dex]);
    const trusted = [okxItem, dex].filter((item): item is TokenMarketCandidate => Boolean(item?.identityVerified));
