...
import { parseAveToken, resolveTokenMarket, tokenAddressEquals, type MarketCapKind, type MarketSource, type TokenMarketCandidate } from "@/lib/token-market";
...
async function getAveBatch(tokens: Array<{ chain: string; address: string }>, updatedAt: string, apiKey = "") {
  ...
  try {
    ...
    const rows = Array.isArray(root.data) ? root.data : Array.isArray(record(root.data).tokens) ? record(root.data).tokens as unknown[] : [];
    const result = new Map<string, TokenMarketCandidate>();
    for (const token of tokens) {
      const payload = { data: rows };
      const parsed = parseAveToken(payload, token.chain, token.address, updatedAt);
      if (parsed) result.set(`${token.chain}:${token.address.toLowerCase()}`, parsed);
    }
    return result;
