...
export function parseAveToken(payload: unknown, chain: string, address: string, now = new Date().toISOString()): TokenMarketCandidate | null {
  ...
  return {
    ...
    logo: text(token.logo_url ?? token.logo), description: text(token.description ?? token.token_introduction ?? token.introduction ?? token.project_intro),
    price: pick(token, ["current_price_usd", "price_usd", "price"]), marketCap: pick(token, ["market_cap"]), fdv: pick(token, ["fdv"]),
...
export function parseGmgnToken(payload: unknown, chain: string, address: string, now = new Date().toISOString()): TokenMarketCandidate | null {
  ...
  return {
    ...
    description: text(token.description ?? token.desc ?? token.bio), price: pick(token, ["price", "price_usd"]),
...
export function resolveTokenMarket(candidates: Array<TokenMarketCandidate | null | undefined>): ResolvedTokenMarket {
  ...
  const blank: TokenMarketCandidate = { source: "dex", chain: "", address: "", pairAddress: "", name: "", symbol: "", logo: "", description: "", price: null, marketCap: null, fdv: null, circulatingSupply: null, totalSupply: null, decimals: null, liquidity: null, volume24h: null, holderCount: null, createdAt: null, updatedAt: new Date().toISOString(), identityVerified: false };
  ...
  const metric = <K extends "price" | "liquidity" | "volume24h" | "createdAt">(key: K) => valid.find((item) => item[key] !== null)?.[key] ?? null;
  return {
    ...base,
    name: valid.find((item) => item.name)?.name ?? "", symbol: valid.find((item) => item.symbol)?.symbol ?? "", logo: valid.find((item) => item.logo)?.logo ?? "", description: valid.find((item) => item.description)?.description ?? "",
    price: metric("price"), marketCap: computedCap ?? fdvOwner?.fdv ?? null, fdv: fdvOwner?.fdv ?? null,
