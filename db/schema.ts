...
export const stablecoinMintEvents = sqliteTable("stablecoin_mint_events", {
  id: integer("id").primaryKey({ autoIncrement: true }), eventKey: text("event_key").notNull(), provider: text("provider").notNull(), chain: text("chain").notNull(), symbol: text("symbol").notNull(), tokenContract: text("token_contract").notNull(), rawAmount: text("raw_amount").notNull(), amount: text("amount").notNull(), amountUsd: real("amount_usd").notNull(), txHash: text("tx_hash").notNull(), logIndex: integer("log_index").notNull(), blockNumber: integer("block_number"), issuer: text("issuer").notNull(), recipientAddress: text("recipient_address"), evidenceType: text("evidence_type").notNull(), issuanceClassification: text("issuance_classification").notNull(), verificationStatus: text("verification_status").notNull().default("unverified"), transactionTo: text("transaction_to"), callSelector: text("call_selector"), zeroAddressTransferLogIndex: integer("zero_address_transfer_log_index"), supplyBeforeRaw: text("supply_before_raw"), supplyAfterRaw: text("supply_after_raw"), supplyDeltaRaw: text("supply_delta_raw"), sourceUrl: text("source_url").notNull(), contractEvidenceUrl: text("contract_evidence_url").notNull(), chainOccurredAt: text("chain_occurred_at").notNull(), discoveredAt: text("discovered_at").notNull(),
