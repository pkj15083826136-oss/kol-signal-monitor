...
while (Date.now() < deadline) {
  try {
    await new Promise((resolve, reject) => {
      ...
      const buffer = []; let lastMessageAt = Date.now(); let settled = false;
      ...
      const timer = setInterval(() => { if (Date.now() - lastMessageAt > 90_000) ws.ping(); void flush().catch((error) => { ws.close(); finish(error); }); }, 30_000);
      ...
      ws.on("message", (payload) => { lastMessageAt = Date.now(); try { const event = JSON.parse(payload.toString()); if (event?.type === "announcement") buffer.push(event); else if (event?.error) throw new Error(`upbit_${event.error.name || "error"}: ${event.error.message || "unknown"}`); } catch (error) { if (error instanceof SyntaxError) return; finish(error); } if (buffer.length >= 20) void flush().catch((error) => finish(error)); });
